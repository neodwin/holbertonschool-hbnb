...
class AmenityList(Resource):
    ...
    @jwt_required()  # Only authenticated users can create a new amenity
    def post(self):
        """Register a new amenity"""
        current_user = get_jwt_identity()

        # Ensure the user is an admin before creating a new amenity
...
class AmenityResource(Resource):
    ...
    @api.response(404, 'Amenity not found')
    @api.response(400, 'Invalid input data')
    @jwt_required()  # Only authenticated users can modify amenities
    def put(self, amenity_id):
        """Update amenity details"""
        current_user = get_jwt_identity()

        # Ensure the user is an admin before updating an amenity
        if not current_user.get('is_admin'):
            return {'error': 'Admin privileges required'}, 403
